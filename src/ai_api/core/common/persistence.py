...
from .errors import AIPersistenceSettingsError, wrap_persistence_error

__all__: list[str] = ["PersistenceManager"]
...
class PersistenceManager:
    """Manages all PostgreSQL persistence for requests/responses.
    ...
    async def get_pool(self) -> Any:
        if self._pool is None:
            if self._pg_resolved_settings is None or PgPoolManager is None:
                ...
                raise AIPersistenceSettingsError(
    ...
    def _build_endpoint(self, request: xAIRequest | OllamaRequest) -> dict[str, Any]:
        return {
            "provider": "xai",
            "model": request.model,
            "host": "api.x.ai",
    ...
    # ------------------------------------------------------------------
    # FULL MULTIMODAL MEDIA SAVING
    # ------------------------------------------------------------------
    async def _save_media_files(
        self, response_id: uuid.UUID, request: xAIRequest | OllamaRequest
    ) -> list[str]:
    ...
    async def save_to_history(
        self,
        request: xAIRequest | OllamaRequest,
    ...
    async def persist_request(
        ...
        if isinstance(request, OllamaRequest):
            meta = {
    ...
    async def persist_response(
        ...
        request: xAIRequest | OllamaRequest | None = None,
        batch_id: str | None = None,
        tree_id: uuid.UUID | None = None,
        branch_id: uuid.UUID | None = None,
        parent_response_id: uuid.UUID | None = None,
